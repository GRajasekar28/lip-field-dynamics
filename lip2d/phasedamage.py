...
class phase_damage_AT2:
    ...
    def crack_surface_functional(self, d):
        ## provided d at faces returns crack suface functional for AT2
        ## csf - same as crack surface area in 3D (or  crack length  in 2D)
        nv = self.mesh.nvertices
        nf = self.mesh.ntriangles
        #csf = np.zeros(nf)
        
        d_vert = np.zeros(nv)  ## damage at each face
        
        for iv in range(nv):
            tris = self.mesh.getVertex2Triangles(iv)
            for tr in tris:
                    #print(t)
                    #np.mean(x[t])
                    d_vert[iv] += d[tr]/len(tris)
        
        grad_d =  (self.gradOp_damage()*d_vert   ).reshape((nf,2))
    ...
    def dissip_fracture(self,d):
        ## provided d at vertices gives dissipation due to fracture
